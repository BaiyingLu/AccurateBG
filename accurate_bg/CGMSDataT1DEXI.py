...
class CGMSData(object):
    ...
    def __init__(self, fmt, filepath, sampling_interval):
        ...
        self.raw_data = reader.read() # Modify this line to use the newest function
    ...
    def _scale(self, standardize):
        if standardize:
            mean = 158.288
            std = 60.565
            self.train_x = ((self.train_x - mean) / std).astype(np.float64)
            self.train_y = ((self.train_y - mean) / std).astype(np.float64)
            self.test_x = ((self.test_x - mean) / std).astype(np.float64)
            self.test_y = ((self.test_y - mean) / std).astype(np.float64)
        else:
            self.train_x = (self.train_x * self.scale).astype(np.float64)
            self.train_y = (self.train_y * self.scale).astype(np.float64)
            self.test_x = (self.test_x * self.scale).astype(np.float64)
            self.test_y = (self.test_y * self.scale).astype(np.float64)
